...
class TPAAppointmentService {
    ...
    static async createBulkAppointments(appointments, tpaContext) {
        ...
        for (let i = 0; i < appointments.length; i++) {
            ...
            const appointmentNumber = appointmentData.applicantion_number || `APP-${i + 1}`;
        ...
        for (const validation of validationResults) {
            try {
                const result = await this.insertAppointmentInDatabase(validation.mappedData, tpaContext);
                results.success.push({
                    success: true,
                    appointment_id: result.appointmentId,
                    case_number: result.caseNumber,
                    application_number: validation.mappedData.applicantion_number,
